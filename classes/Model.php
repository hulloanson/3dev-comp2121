...
class Model
{
  ...
  protected function is_relation($name) {
    if (isset(static::$has_many[$name])) return 'has_many';
    else if (isset(static::$has_many_through[$name])) return 'has_many_through';
    else if (isset(static::$belongs_to[$name])) return 'belongs_to';
    else return false;
  }
  ...
  public function __get($name)
  {
    if (!isset($this->data[$name])) {
      ...
      if (!$relation) {
        ...
        if (($parent_id= $this->{"${class_table}_id"}) === null) return null;
  ...
  public function save()
  {
    ...
    foreach ($this->data as $prop => $value) {
      if ($this->is_relation($prop)) continue;
      if ($first) {
        $first = false;
      } else {
        $cols .= ',';
